from ._anvil_designer import shear_cmTemplate
from anvil import *
...
class shear_cm(shear_cmTemplate):
  def __init__(self, **properties):
    ...
    self.card_data = BlockCard(
      title="Données — Cisaillement",
      header_color="input",  # jaune
    )

    # --- Inputs principaux (toujours visibles) ---
    self.row_ved = RowItem("Ved", value=500, editable=True)
    self.row_fy = RowItem("fy", value=235, editable=True)
    self.row_Av = RowItem("Av", value=10, editable=True)
    self.card_data.add_input(self.row_ved)
    self.card_data.add_input(self.row_Av)
  ...
  def on_change_select_sec(self, **event_args):
    ...
    for val in param:
      ...
      if val == "Av":
        self.row_sec_Av = RowItem(val, value=value, editable=False, row_type="param")
        row = self.row_sec_Av
      else:
  ...
  def on_checked(self, **event_args):
    if self.row_checked.checked:
      self.row_Av.tb_value_enabled = False
      self.row_Av.value = self.row_sec_A.value

    else:
      self.row_Av.tb_value_enabled = True
  ...
  def calculer(self, **event_args):
    ...
    payload = {
      ...
      "Av": float(self.row_Av.tb_value.text),
      "load": norme.convert_unit(float(self.row_ved.tb_value.text), "kN", "N"),
    }
    response = norme.api_call(API_URL, payload)

    formula = response["vc_rd"]["formula"]
    ved = float(self.row_ned.tb_value.text)
    vrd = norme.convert_unit(response["vc_rd"]["result"], "N", "kN")
    ...
    row = RowItem(
      name=response["v_rd"]["name"],
      value=f"{vrd:.2f}",
      unit="kN",
      formula=f"{response['vc_rd']['formula']} \n {response['nv_rd']['formula_values']}",
      ref=response["vc_rd"]["ref"],
      editable=False,
      row_type=row_type,
    )
    ...
    self.graph_rslt = RowPlot(val=(ved / vrd))
